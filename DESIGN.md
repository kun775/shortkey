...
> **本项目适配（sk.gs）**
> 本文件是 sk.gs 的设计基准，取自 Linear 的设计系统。以下为本项目落地时的取舍，未在本表列出的条目一律按原规范执行。
>
> | 项 | Linear 原规范 | 本项目采用 | 原因 |
> | --- | --- | --- | --- |
> | 明暗模式 | dark-mode 原生（`#08090a` 为画布） | **亮色为主 + 完整暗色**（`dark:` class 切换） | 短链工具多用于白天办公场景；两套色板均按本文件「Light Mode Neutrals」与「Background Surfaces」落地 |
> | 等宽字体 | Berkeley Mono | **JetBrains Mono** | 已是项目既有依赖，无需新增字体加载 |
> | 强调色 | `#5e6ad2`（bg）/ `#7170ff`（accent）/ `#828fff`（hover） | 同色，映射为 Tailwind `brand-*` 色阶 | 与原 indigo 主色连续，视觉平滑迁移 |
> | 组件基础 | Radix UI primitives | 原生 Tailwind + 少量内联交互 | 项目零 UI 依赖，保持边缘场景下的轻量 |
> | 字重档位 | 300 / 400 / 510 / 590 | 400 / 500 / 600（非可变字体环境下的最近似） | Google Fonts 加载的是 Inter 静态字重；`510` 以 `500` 近似，`590` 以 `600` 近似 |
>
> 字体特性 `"cv01", "ss03"` 与负字距规则按原规范执行。
